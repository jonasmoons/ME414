...
2.  Working with the `dplyr` package
    ...
    a.  Load the Stata dataset used in this paper, available [here](http://www.kenbenoit.net/files/dail2002.dta).  To load this into R, you will need the `read.dta` command from the `foreign` package.  (Note that you can load straight from the URL using this command.)  Call this data object `dail2002`.  What sort of object is this?  How can you tell what sort of object it is? Note that before you can execute this code, you will need to run `install.packages("foreign")` in the console to install the `foreign` package (unless you have done so already). The same goes for other packages in this exercise.
